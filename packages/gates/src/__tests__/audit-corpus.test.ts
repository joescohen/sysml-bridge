...
describe("buildResolutionSet — n2Interfaces extension", () => {
  ...
  it("provenanceFindings emits no error for provenanceSourceId matching an n2 id", async () => {
    const { provenanceFindings } = await import("../provenance.js");
    const s = buildResolutionSet(N2_EXTRACTED as any);
    const elements = [
      {
        id: "elem-001",
...
// The ANGARS corpus IR baseline is committed at examples/angars/extracted.json
// (public by design — see CLAUDE.md). Always present in CI; run unconditionally.
const EXTRACTED_PATH = path.resolve(__dirname, "../../../../examples/angars/extracted.json");
