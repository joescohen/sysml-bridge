...
export interface ProseComposedIR {
  ...
  /**
   * Optional chunkId → chunk text map for the ingested source documents.
   *
   * When present, the Gate-1 audit re-checks each approved prose entry's
   * citation.quote against the text of its citation.chunkId
   * (PROSE-unverbatim-quote). When ABSENT the gate degrades to the
   * chunk-store-unavailable path (a warning, never a vacuous pass) — exactly the
   * way corpus-null degrades GATE03. composeIR() does not populate this field
   * (chunk text is not persisted in the two-layer files); a caller that has the
   * ingest chunk store attaches it before auditing.
   */
