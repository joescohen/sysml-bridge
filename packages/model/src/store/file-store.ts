...
// (ProjectDescriptor replaces the former SMAPS wire type)
import {
...
export class FileStore implements ModelStore {
  ...
  async createProject(name: string): Promise<ProjectDescriptor> {
    ...
    this.elements = [];
    await this.persist();
    return this.toProjectDescriptor();
  }
  ...
  private toProjectDescriptor(): ProjectDescriptor {
    return {
      "@id": this.projectId!,
      "@type": "Project",
  ...
  private async persist(): Promise<void> {
    ...
    // Atomic write: a crash mid-write must never corrupt the only copy of the
    // model. Write to a temp file in the same directory, then rename over the
    // target (rename is atomic on POSIX filesystems within one mount).
...
// Exported (not just used internally) so other ModelStore backends — e.g.
// SysmlV2ApiStore, whose API has no server-side "give me all Relationship
// elements" query the pilot actually supports — can classify relationships
// client-side with the exact same heuristic, instead of duplicating it.
export function isRelationship(el: SysmlElement): boolean {
...
export function toRelationship(el: SysmlElement): SysmlRelationship {
  return {
    id: el.id,
    type: el.type,
