...
describe("G-C / C5 — no vector/embedding/retrieval in ingest path", () => {
  it("grep finds no lancedb|embedding|vector|topK|top-k|rerank in scripts or candidates/src/prose", () => {
    const repoRoot = join(import.meta.dirname, "../../../../../");
    const scriptPath = join(repoRoot, "scripts");
    const srcPath = join(repoRoot, "packages/candidates/src/prose");

    let output = "";
    let errored = false;
    try {
      // Excludes __tests__ dirs and comment-only lines: this proves no
      // retrieval/embedding/vector-DB *calls* exist in implementation source,
      // not that the words never appear in a docstring/test-title explaining
      // their absence (several files document the C5 no-retrieval invariant,
      // and this very test's own title/pattern string would otherwise
      // self-match).
      output = execSync(
        `grep -rEn --exclude-dir=__tests__ "lancedb|embedding|vector|topK|top-k|rerank" "${scriptPath}" "${srcPath}" 2>/dev/null | grep -Ev '^[^:]+:[0-9]+: *(\\*|//|/\\*)' || true`,
        { encoding: "utf8" },
      );
    } catch {
      errored = true;
