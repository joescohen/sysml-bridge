...
function makeProvider(proposals: CandidateProposal[]): LlmProvider {
  return {
    async propose(chunkId: string, chunkText: string): Promise<CandidateProposal[]> {
      // Stamp all proposals with the real chunkId so the C4 citation gate passes,
      // and a verbatim quote drawn from the chunk text so the C6 verbatim gate
      // passes — this suite exercises the T1 kind-specific-field gate, not C6.
      const quote = chunkText.slice(0, 24);
      return proposals.map((p) => ({ ...p, citedChunkId: chunkId, quote }));
    },
  };
}
