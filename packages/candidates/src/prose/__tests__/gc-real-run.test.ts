...
class OfflineMockProvider implements LlmProvider {
  async propose(
    ...
    // Single deterministic proposal that resolves to the actual chunk. The quote
    // is a genuine span of the chunk text so the C6 verbatim gate passes (a mock
    // must not fabricate a quote absent from its chunk).
    return [
      {
