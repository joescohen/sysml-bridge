...
// ── Non-PDF heading-driven extraction ──────────────────────────────────────────

/**
 * A heading known EXACTLY from a format's native structure (markdown `#`,
 * DOCX heading styles, XLSX sheet name, TXT page/paragraph fallback) —
 * structurally compatible with `ParsedHeading` from parsers/types.ts, but
 * declared locally so section-map.ts stays independent of the parsers dir.
 */
export interface HeadingInput {
  title: string;
  level: number;
  /** Index into the source RawParseResult.pages array where this heading starts. */
  pageIndex: number;
}

/**
 * Build a SectionMap directly from a parser-supplied heading list (DOCX,
 * XLSX, CSV, MD, TXT) — no text heuristics involved, since the format
 * already tells us exactly where sections start. Mirrors
 * `extractSectionMapFromPages`'s tree-building (same nesting-by-level rule),
 * just skipping the PDF-only numbered/ALL-CAPS detection step.
 */
export function extractSectionMap(
  headings: HeadingInput[],
  documentId: string,
  format: SectionMap["format"],
  totalPages?: number,
): SectionMap {
  const flatHeadings: FlatHeading[] = headings.map((h) => ({
    title: h.title,
    level: h.level,
    pageStart: h.pageIndex,
  }));
