...
import type { ContextBundle, OfferedFact, RetrievedChunk } from "./types.js";
...
// ── Lexical retrieval integration (BM25 over the chunk store) ────────────────

/** Top-k retrieved chunks per side (source, target). */
const RETRIEVE_PER_SIDE = 5;
/** Retrieved chunk text truncation in the offered-fact detail line. */
const RETRIEVED_DETAIL_CHARS = 200;

/**
 * Collect the query terms for one element: its own name + the names of its 1-hop
 * IR neighbors. Reuses collectOfferedFacts (called with id on both sides so the
 * `seen` dedup yields exactly the element + its neighborhood).
 */
function collectQueryTerms(id: string, ir: InferredComposedIR): string {
  return collectOfferedFacts(id, id, ir)
    .map((f) => f.name)
    .join(" ");
}

/**
 * Retrieve corpus evidence for a candidate pair via BM25: query the index once
 * per side (source terms, target terms), take top-k each, and dedup by chunkId.
 * Deterministic: query terms derive from the IR, BM25 ordering is score-then-
 * chunkId, and the merge preserves source-first then target order.
 */
export function retrieveEvidence(
  ...
  const sides: Array<[string, string]> = [
    [collectQueryTerms(sourceId, ir), "source"],
    [collectQueryTerms(targetId, ir), "target"],
  ];
  for (const [query] of sides) {
    for (const hit of index.query(query, RETRIEVE_PER_SIDE)) {
 ...
 * (the premise id contract), relevant exact-id corpus quotes, and — when a BM25
 * chunk-store index is supplied — top-k retrieved corpus passages merged in as
 * clearly-labeled evidence and as citable `kind: "chunk"` offered facts.
 *
 * Determinism: with a fixed IR (and fixed index), the returned bundle is a pure
 * function of (sourceId, targetId) — same inputs → byte-identical bundle.
 *
 * Backward-compatible: called without `index`, behavior is exactly as before
 * (no retrievedEvidence, offeredFacts unchanged).
 */
export function buildContextBundle(
  ...
  const offeredFacts = collectOfferedFacts(sourceId, targetId, ir);

  // BM25-retrieved evidence (only when a chunk store was supplied). Each hit is
  // appended as a citable offered fact (id = chunkId) so premises citing it
  // RESOLVE — the engine adds these ids to the resolvable set (§validatePremises).
  const retrievedEvidence = index ? retrieveEvidence(sourceId, targetId, ir, index) : [];
  const offeredIds = new Set(offeredFacts.map((f) => f.id));
  for (const hit of retrievedEvidence) {
    if (offeredIds.has(hit.chunkId)) continue;
    offeredIds.add(hit.chunkId);
    offeredFacts.push({
      id: hit.chunkId,
      kind: "chunk",
      name: hit.sectionPath,
      detail: hit.text.replace(/\s+/g, " ").trim().slice(0, RETRIEVED_DETAIL_CHARS),
    });
  }
...
// ── Cross-document context bundle (W2 co-occurrence / chain candidates) ───────

/** Minimal descriptor of a cross-document candidate for context assembly. */
export interface CrossDocContextInput {
  sourceId: string;
  targetId: string;
  relationFamily: string;
  /** Co-occurring chunk ids (co-occurrence) or constituent relation ids +
   *  evidence chunks (chain) — the candidate's citable premises. */
  premiseIds: readonly string[];
  origin: "cooccurrence" | "chain";
}

/**
 * Build a ContextBundle for a cross-document candidate whose endpoints are
 * CANONICAL ENTITY IDS (not composed-IR ids). The IR-centric `buildContextBundle`
 * cannot resolve entity endpoints, so this offers the two entity endpoints plus
 * each premise id as citable offered facts — the propose prompt renders them as
 * `[id: …]` lines and the engine admits the same ids as resolvable premises, so
 * a proposal citing them is queued rather than dropped_unpremised.
 *
 * Determinism: pure function of the input descriptor (+ entity name lookup).
 */
export function buildCrossDocContextBundle(
  input: CrossDocContextInput,
  entityNames: ReadonlyMap<string, string>,
): ContextBundle {
  const offeredFacts: OfferedFact[] = [];
  const seen = new Set<string>();
  const add = (fact: OfferedFact) => {
    if (seen.has(fact.id)) return;
    seen.add(fact.id);
    offeredFacts.push(fact);
  };

  const srcName = entityNames.get(input.sourceId) ?? input.sourceId;
  const tgtName = entityNames.get(input.targetId) ?? input.targetId;
  add({ id: input.sourceId, kind: "entity", name: srcName, detail: `cross-doc ${input.origin} source` });
  add({ id: input.targetId, kind: "entity", name: tgtName, detail: `cross-doc ${input.origin} target` });
  for (const pid of input.premiseIds) {
    add({
      id: pid,
      kind: input.origin === "chain" ? "relation-or-chunk" : "chunk",
      name: pid,
      detail: `${input.origin} evidence premise`,
    });
  }

  return {
    sourceNeighborhood: `[ENTITY] ${srcName} (id: ${input.sourceId}) — cross-doc ${input.origin} source`,
    targetNeighborhood: `[ENTITY] ${tgtName} (id: ${input.targetId}) — cross-doc ${input.origin} target`,
    corpusQuotes: [],
    offeredFacts,
  };
}
