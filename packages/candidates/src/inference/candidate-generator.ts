...
import { zeroFamilyCounts } from "./types.js";

// ── Stable ID for inference candidates ──────────────────────────────────────

/**
 * Deterministic stable id for a candidate: sha256 of "infer:<family>:<src>:<tgt>",
 * returns "infer-<hex16>" — same approach as stableId in @sysml-bridge/model.
 */
...
export function generateCandidates(
  ...
  // The structural generator only emits the four intra-document families; the
  // trace families (satisfy/derive/verify) are enumerated over the entity store
  // (cooccurrence.ts), not here. zeroFamilyCounts() seeds all families so the
  // record type-checks and downstream stats never see an undefined key.
  const countsByFamily = zeroFamilyCounts();
